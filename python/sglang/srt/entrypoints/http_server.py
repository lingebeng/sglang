...
"""
ZeroMQ 是一个高性能异步消息队列库。可以把它理解为增强版的 socket：                                                                                              
                                                                  
  - 普通 socket：你需要自己处理连接、重连、消息边界、多对多路由                                                                                                   
  - ZMQ：帮你封装好了这些，直接 send() / recv() 消息就行          
                                                                                                                                                                  
  SGLang 中为什么用 ZMQ                                                                                                                                           
                                                                                                                                                                  
  SGLang 的三个核心组件跑在不同进程里：                                                                                                                           
                                                                  
  主进程                    子进程1                  子进程2                                                                                                      
  ┌──────────────┐         ┌──────────────┐         ┌──────────────┐                                                                                              
  │ TokenizerMgr │──ZMQ──▶ │  Scheduler   │──ZMQ──▶ │ DetokenizerMgr│
  │  (tokenize)  │◀──ZMQ── │  (GPU推理)    │◀──ZMQ── │  (detokenize)│                                                                                              
  └──────────────┘         └──────────────┘         └──────────────┘                                                                                              
                                                                                                                                                                  
  进程之间不能直接共享内存（和线程不同），所以需要一种**进程间通信（IPC）**机制。常见选择：                                                                       
                                                                                                                                                                  
  ┌───────────────────────────────────┬────────────────────────────────────────────┐                                                                              
  │               方式                │                    特点                      │
  ├───────────────────────────────────┼────────────────────────────────────────────┤                                                                              
  │ 管道/队列 (multiprocessing.Queue) │ 简单但性能一般                                 │
  ├───────────────────────────────────┼────────────────────────────────────────────┤
  │ 共享内存                          │ 快但只能传固定结构                             │                                                                              
  ├───────────────────────────────────┼────────────────────────────────────────────┤                                                                              
  │ gRPC / HTTP                       │ 通用但开销大（序列化+网络栈）                   │                                                                              
  ├───────────────────────────────────┼────────────────────────────────────────────┤                                                                              
  │ ZMQ IPC                           │ 接近共享内存的速度，接口像 socket 一样简单       │
  └───────────────────────────────────┴────────────────────────────────────────────┘                                                                              
                                                                  
  SGLang 选 ZMQ 的原因：低延迟 + 支持异步 + 支持多种通信模式（请求-响应、发布-订阅等）。                                                                          
                                                                  
  IPC 模式                                                                                                                                                        
                                                                  
  ZMQ 的 IPC 传输用的是 Unix domain socket（ipc:///tmp/xxx），数据不经过网络栈，直接在内核态拷贝，比 TCP 快很多。                                                 
                                                                  
  简单说：ZMQ 就是一个让不同进程之间高效传消息的库，SGLang 用它来连接 Tokenizer、Scheduler 和 Detokenizer 三个进程。
"""
